...
- `references/api.md`：画布、图形、文字、填充、连接、完整 Lucide 与 lint API；
- `references/layout.md`：布局、留白、**方框 vs 图形化选型**、复杂连线；
...
`allowCrossing: true`；不要为了消除报告而全局关闭检查。
...
1. **语义几何 lint**：已登记节点/图标/连线的区域重叠、线穿节点或图标、线线交叉/共线、越界；
...
2. 文字与图标是否可读，有无截断、压线、图标互压或压住卡片文字；
