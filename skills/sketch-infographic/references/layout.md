...
信息图不是正文排版：读者远看先认结构，近看再读细节。**不要整图默认 `size: 18`。**
落笔前先定文字角色，再用不同 `size`（可配 `weight` / `GRAY`）拉开层次：

| 角色 | 建议 `size` | API 提示 |
|------|------------|----------|
| 主标题 | `28`～`31` | `header` 已内置；自制标题用 `text` + `weight: 700` |
| 分区 / 组标题 | `20`～`22` | 区域角标、列头 |
| 节点名 / 卡片正文 | `16`～`18` | `card` / `inBox` |
| 注解 / 连线标签 | `12`～`14` | `wireLabel`；`connect` 用 `labelSize` |

相对落差比绝对像素更重要：相邻两级至少差约 `3`～`4px`，否则缩略图里分不清主次。
关键层级设 `fit: false`，或先 `measure` 加宽卡片，避免自动缩字把层次抹平。
内容过密时加高画布或拆图，不要全局压字号。

## 四、常见问题与修法

| 现象 | 原因 | 修法 |
|------|------|------|
| 主次看不清 | 全图同字号 | 按上节分层设 `size` / `labelSize`，标题加 `weight: 700` |
...
| 箭头贴着节点底/顶边横穿 | `bypass` 走廊方向与端口边平行，或正交 L 弯穿框 | 用 `bypass`（自动 `approach` 退让）或让 `connect` 走外侧；看 lint「净空不足」 |
| 底部元素被截断 | 画布高度不够 | 画布高度 = 最低元素底边 + 40～60；不加 `footer` 时不要额外预留图注空白 |
| 底部大片留白 | 为已删除的 `footer` 仍留高度 | 按内容下沿收紧 `height`，不要默认加图注区 |
| 自由轮廓形状不受控 | 手写贝塞尔控制点凑形状 | 改成参数化半宽函数，采样点平滑连成 path，见 SKILL.md「形态当主体」 |
| 内部构件戳出轮廓 | 用起点处的宽度算了终点 | 用终点位置的宽度，跨度大时取两端 `Math.min` |
| 器官上叠着主脉或网格 | 底层纹理透过来 | 先用 `PAPER` 不透明填充盖一层，再补回底色，最后画外膜 |
...
当走廊方向与端口边平行（如 `via: 'left'` 却从南端口进入）时，库只在**终点**把平行长段退到
`approach`（默认 16px）净空，再以短 stub 垂直扎入，避免贴边横穿；起点不再额外折弯。
左绕行时出发侧宜用 `w`（右绕行用 `e`），路径更顺。
...
第一层会检查矩形重叠、图标互压/压节点、线穿非目标节点或图标、**贴边擦线（净空不足）**、
线线交叉/共线和画布越界；
...
2. 逐块看文字与图标，有没有被缩小、遮挡、切断，图标是否互压或挡住连线；
   **主次字号是否分明**（标题 > 节点 > 注解，且未被 `fit` 压成一档）；
